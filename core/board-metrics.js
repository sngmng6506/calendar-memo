...
})(typeof globalThis !== "undefined" ? globalThis : this, function createBoardMetrics() {
const dayMs = 24 * 60 * 60 * 1000;
const energyMax = 120;

function getTodaySummary(board, date) {
  const targetDate = date || getKoreaDateKey();
  const tasks = Array.isArray(board && board.tasks) ? board.tasks : [];
  const items = tasks
    .filter((task) => !isCompanyProjectTaskType(task))
    .filter((task) => hasTaskWorkOn(task, targetDate))
    .map((task) => ({
      id: task.id,
      name: task.name || "Untitled",
      fixed: Boolean(task.fixed),
      workload: roundOne(getTaskWorkloadOn(task, targetDate)),
    }));

  const workload = roundOne(items.reduce((sum, item) => sum + item.workload, 0));
...
function hasTaskWorkOn(task, date) {
  ...
  if (task.fixed) return isWorkday(date) || Boolean(entries[date]);
...
function getRecurringEntry(task, date) {
  ...
  return { recurring: true, note: "Recurring" };
}
...
function getMilestonesForDate(board, date) {
  const tasks = Array.isArray(board && board.tasks) ? board.tasks : [];
  return tasks
    .filter((task) => task && !isCompanyProjectTaskType(task))
...
function getTaskProgressThroughDate(task, date) {
  ...
  return clamp(
    Object.entries(entries).reduce((sum, [entryDate, entry]) => {
      ...
      return sum + Number((entry && entry.delta) || 0);
    }, 0),
    0,
    100,
  );
