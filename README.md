...
- 월 제목과 이전·다음·오늘 이동은 화면 가로 중앙에 고정됩니다.
- 날짜는 카드나 흰 칸으로 나누지 않고 7열 정렬과 여백만으로 구분합니다.
- 빈 날짜에는 입력 상자와 체크박스가 보이지 않습니다. 날짜 영역을 클릭하면 바로 입력 커서가 나타납니다.
- 저장된 업무만 `□ / ✓`와 텍스트로 표시합니다.
- 기본 창 불투명도는 `0.86`이며 창 합성을 지원하지 않는 환경에서는 불투명하게 표시됩니다.
