...
- 미완료 복사는 원본을 삭제하지 않는다.
- 최초 업무의 `origin_task_id`는 `NULL`이고 복사본은 최초 원본의 ID를 갖는다.
- 같은 대상 날짜에는 동일한 `origin_task_id`의 복사본을 두 번 만들지 않는다.
- 복사 여부는 계보에서 날짜가 가장 최근인 업무의 완료 상태로 판단한다.
